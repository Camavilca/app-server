...
# EMPRESA{data-icon="fa fa-home fa-fw"}
...
# datos11 <- read.table(file.choose(),T)     #    datos1_1    info de trabajadores
# datos22 <- read.table(file.choose(),T)     #    datos1_2   puntajes por puesto
# valores <- read.table(file.choose(),T)     #    datos2    tabla de factores

# datos <- read_excel("C:/AVANCE HCP/CREACION DASHBOARD/datos.xlsx")  DATA DE PRUEBA SIN CREAR PLANILLA

#datos11 <- read_excel("C:/AVANCE HCP/prueba data ronil 2/datos1_1.xlsx")  # info de #trabajadores
#datos22 <- read_excel("C:/AVANCE HCP/prueba data ronil 2/datos1_2.xlsx") # puntajes por #puesto
#valores <- read_excel("C:/AVANCE HCP/prueba data ronil/datos2.xlsx")   # tabla de #factores
...
datBS <- datos %>% select(CODIGO,GENERO,CARGO,Puntos,Ingreso)
names(datBS) <-c("CODIGO", "GENERO" , "Puesto" , "Puntaje", "Sueldo" )

needs(plotly)
Disp = ggplot(data = datBS,aes(x=Puntaje,y=Sueldo))+geom_point(alpha=0.6)+theme_bw()+
  labs(title = "")+ xlab("Valoracion de puestos")+ylab("Remuneración")
# +stat_smooth(colour = "gray80",level = 0)
ggplotly(Disp)


```

### El gráfico de dispersion entre la valoracion de puestos y la remuneracion.

```{r }

spamM <- ""
valueBox(spamM, icon = "fa-address-cardr", color = 'steelblue')


```

# BANDAS{data-icon="fa fa-square"}
...
p= p+theme(axis.text.x=element_blank(),axis.ticks.x=element_blank())+ylab("Remuneración")+xlab('Valoración de puestos')+ scale_y_continuous(limits = c(0, LIM_y))
#c("#FF4000","#013ADF")
if(max(datB$Caja)==3){
...
### PROPUESTAS DE BANDAS SALARIALES

```{r }
cr <- c()
for(c in 1:length(ff)){
  f1 <- round(ff[c]*0.65,0)
  f2 <- round(ff[c]*1.35,0)
  f12 <- rbind(f1,f2)
  cr <- cbind(cr,f12)
}

BNS <- c("BANDA1","BANDA2","BANDA3","BANDA4","BANDA5","BANDA6")
BNS <- BNS[1:length(ff)]

row.names(cr) <- c("LimiteInferior","LimiteSuperior")
colnames(cr) <- BNS


rr1 <- as.numeric(round(c(tapply(datB$SUELDO, datB$Caja, min)),0))
rr2<- as.numeric(round(c(tapply(datB$SUELDO, datB$Caja, median)),0))
rr3 <-  as.numeric(round(c(tapply(datB$SUELDO, datB$Caja, mean)),0))
rr4 <-  as.numeric(round(c(tapply(datB$SUELDO, datB$Caja, max)),0))

cr2 <- rbind(rr1,rr2,rr3,rr4)

BNS <- c("BANDA1","BANDA2","BANDA3","BANDA4","BANDA5","BANDA6")
BNS <- BNS[1:length(ff)]

row.names(cr2) <- c("IngresoMinimo","Mediana","Promedio","IngresoMaximo")
colnames(cr2) <- BNS

cr2_med<- cr2[2,]
cr <- rbind(cr[1,] ,cr2_med , cr[2,])

row.names(cr) <- c("LimiteInferior","Mediana","LimiteSuperior")
colnames(cr) <- BNS



needs(DT)
datatable(cr, options = list(dom = 't'))


```

### ANALISIS DESCRIPTIVO DE LA PLANILLA POR BANDA
...
cr2 <- cr2[-2,]
needs(DT)
datatable(cr2, options = list(dom = 't'))

```

# INCUMPLIMIENTO{data-icon="fa-users"}

## Column

### INFERIOR : Cuando la compensacion total del trabajador es menor al limite inferior de la banda

```{r }

spamM <- ""
valueBox(spamM, icon = "fa-address-card", color = 'red')


```

### SUPERIOR : Cuando la compensacion total del trabajador es mayor al limite superior de la banda
...
### OBSERVACIONES FUERA DE LAS BANDAS

```{r }
needs(dplyr)
dattt <- datos %>% select(CODIGO,GENERO,CARGO,Puntos,Ingreso)
names(dattt) <-c("CODIGO", "GENERO" , "Puesto" , "Puntaje", "Sueldo" )

needs(stringr)
dattt$GENERO <- chartr('áeioúÁeioÚ','aeiouAEIOU',dattt$GENERO)
dattt$GENERO <- str_to_upper(dattt$GENERO, locale = "es")

for (gen in 1:nrow(dattt)) {
  dattt[gen,2] <- ifelse(dattt[gen,2]=="HOMBRE","MASCULINO",dattt[gen,2])
  dattt[gen,2] <- ifelse(dattt[gen,2]=="MUJER","FEMENINO",dattt[gen,2])
  dattt[gen,2] <- ifelse(dattt[gen,2]=="F","FEMENINO",dattt[gen,2])
  dattt[gen,2] <- ifelse(dattt[gen,2]=="M","MASCULINO",dattt[gen,2])
}  ; rm(gen)
...
Cj <- data.frame(Cj,ff)
names(Cj) <- c("Caja","LimInf","LimSup_M","LimInf_H","LimSup","Medi") ;row.names(Cj) <- NULL
Cj <- Cj %>% mutate(Med_T=Medi*1.35 , Med_B=Medi*0.65)
Cj <- Cj %>% select(Caja, LimInf, LimSup, Medi ,Med_T, Med_B)
Cj$Med_T <- round(Cj$Med_T,0)
Cj$Med_B <- round(Cj$Med_B,0)
dr1 <- merge(bas_1,Cj,by="Caja",by.x = "Caja", by.y = "Caja")

puntos_fuera <- function(x,sup,min){
  kl <- c()
  for (m in 1:length(x)) {
    kl[m] <- ifelse((x[m]>sup[m] || x[m]<min[m]),1,0) }
  return(kl)}
OBS=puntos_fuera(dr1$Sueldo,dr1$Med_T,dr1$Med_B)

dr1 <- data.frame(dr1,OBS)

bastt_fuera <- dr1 %>% subset(dr1$OBS==1)

if(nrow(bastt_fuera)!=0){

Categoria <- rep("Inferior",nrow(bastt_fuera))

for (m in 1:nrow(bastt_fuera)) {
  Categoria[m] <- ifelse(bastt_fuera$Sueldo[m]>bastt_fuera$Med_T[m],"Superior",Categoria[m]) }

bastt_fuera1 <- cbind(bastt_fuera,Categoria)

bastt_fuera2 <- bastt_fuera1  %>% select( Caja, CODIGO,GENERO ,Puesto, Puntaje,Sueldo,Categoria)
names(bastt_fuera2) <- c("Banda","Codigo","Genero","Puesto","Puntaje", "Sueldo", "Estado")

bastt_fuera2$Sueldo <- round(bastt_fuera2$Sueldo,0)
bastt_fuera2$Puntaje <- round(bastt_fuera2$Puntaje,0)


needs(DT)

datatable(bastt_fuera2, options = list(bPaginate = FALSE, columnDefs = list(list(className = 'dt-center', targets = c(1,2,3,5,6,7)  ))))%>% formatStyle('Estado', color = 'white',  backgroundColor = styleEqual(c("Superior","Inferior"), c('steelblue', 'red')))
} else {
  print("No se tiene observaciones fuera")
}

```

# COMPORTAMIENTO{data-icon="fa-line-chart"}
...
datos_mod$GENERO <- chartr('áéíóúÁÉÍÓÚ','aeiouAEIOU',datos_mod$GENERO)
...
# if (min(SUM)<300){min_x2=220} else{min_x2=290}

#######################
#####     Grafica del mejor modelo
#######################

# x2 <- sort(datos_mod$PUNTOS)
x2 <- seq(200,1000,2.5)
...
rree11 <- str_c("El comportamiento de la valoración de puestos y la remuneración de los trabajadores se ajustan a ",Ecuacion," el cual tiene un R2 de ",R22, " indicando que es un buen modelo.")
rree11 <- as.data.frame(rree11)

datatable(rree11, options = list(dom = 't'),rownames = FALSE,colnames = NULL)
...
# fg <- seq(1,length(ff),1)
# MEDIANAS_TOTAL <- data.frame(CAJA=fg,MEDIANAS=ff)
# MEDIANAS_TOTAL
...
# TABLA RESUMEN{data-icon="fa fa-list-ol"}

## Column{data-width=550}

### PROPORCIONES DE PUESTOS OBSERVADOS
...
if(mean(dr1$OBS)==0){
  print("No se tiene puestos observados")
} else {

qq <- table(dr1$Puesto,dr1$OBS)
qq <- as.matrix(qq)
qq2 <- as.numeric(qq[,2])

ttt=dr1%>%  group_by(Puesto) %>%  summarize(Nn=n())

dr2 <- cbind(ttt,qq2)
dr2 <- dr2 %>% select(Puesto,qq2,Nn)

Base_nueva <- dr2 %>% mutate(qq2_p=round((qq2/Nn)*100,2),Nn_p= round((Nn/nrow(dr1))*100,3))

names(Base_nueva)=c("CARGO","CASOS OBSERVADOS","TOTAL POR CARGO","PROPORCION DEL CARGO","PROPORCION DEL TOTAL")

Base_nueva=Base_nueva[order(-Base_nueva$`PROPORCION DEL TOTAL`),]

rownames(Base_nueva) <- Base_nueva$CARGO
Base_nueva$CARGO<- NULL


needs(DT)
datatable(Base_nueva, options = list(bPaginate = FALSE,dom = 't',columnDefs = list(list(className = 'dt-center', targets =1:3)))) %>% formatStyle('PROPORCION DEL TOTAL',
  background = styleColorBar(Base_nueva$`TOTAL POR CARGO`, 'steelblue'),
...
# EQUIDAD{data-icon="fas fa-venus-mars"}

## Column{data-height=550}

### PROPORCION DE LOS HOMBRES FUERA DE LAS BANDAS

```{r }
if(nrow(bastt_fuera)!=0){
hoB=subset(bastt_fuera2,bastt_fuera2$Genero=="MASCULINO")
hoB_T=subset(dr1,dr1$GENERO=="MASCULINO")

spamH <- round(nrow(hoB)/nrow(hoB_T),3)*100
valueBox(paste(spamH,"%"), icon = "fa-male",color = '#58A1DA')
# deepskyblue
} else {
  print("No se tiene puestos observados")}
```

### PROPORCION DE MUJERES FUERA DE LAS BANDAS

```{r }
if(nrow(bastt_fuera)!=0){
muJ=subset(bastt_fuera2,bastt_fuera2$Genero=="FEMENINO")
muJ_T=subset(dr1,dr1$GENERO=="FEMENINO")

spamM <- round(nrow(muJ)/nrow(muJ_T),3)*100
valueBox(paste(spamM,"%"), icon = "fa-female", color = '#FFAF33')
# firebrick
} else {
  print("No se tiene puestos observados")}
```
...
names(resultado1) <- c("Mediana de mujeres","Mediana de hombres","P-valor","Conclusión")
needs(DT)
datatable(resultado1,options = list(bPaginate = FALSE,dom = 't',columnDefs = list(list(className = 'dt-center', targets =1:3))))

```

### Se aplico la Prueba Estadística de Wilcoxon para la diferencia de medianas. Un P-valor mayor a 0.05 nos permite concluir que la mediana de los ingresos de los hombres y de las mujeres son similares en la banda salarial correspondiente.

```{r }

spamM <- ""
valueBox(spamM, icon = "fa-chart-bar", color = 'gray')
...
### EQUIDAD REMUNERATIVA POR GENERO
...
if( is.na(max(na.omit(Cj$Med_T_M))) || is.na(max(na.omit(Cj$Med_T_H))) ){
  if (is.na(max(na.omit(Cj$Med_T_M)))) {
      LIM_y=max(na.omit(Cj$Med_T_H))
  } else {
      LIM_y=max(na.omit(Cj$Med_T_M)) }
} else {
LIM_y <-ifelse( max(na.omit(Cj$Med_T_M)) > max(na.omit(Cj$Med_T_H))  ,max(na.omit(Cj$Med_T_M))+1500, max(na.omit(Cj$Med_T_H))+1500)}
...
# PUESTOS{data-icon="fa fa-dot-circle-o"}

## Column{data-width=550}

### DISTRIBUCION DEL PRESUPUESTO DE LA EMPRESA POR PUESTO DE TRABAJO

```{r }
needs(gapminder);needs(dplyr);needs(ggplot2);needs(wesanderson)
datBS$Sueldo <- gsub(",","", datBS$Sueldo)
datBS$Sueldo <- as.numeric(datBS$Sueldo)

posicion <- names(table(datBS$Puesto))
posicion <- as.character(posicion)
datBS$Puesto <- as.character(datBS$Puesto)

Presupuesto <- round((as.numeric(tapply(datBS$Sueldo, datBS$Puesto, sum))/sum(datBS$Sueldo))*100,3)
Promedio <- round(c(tapply(datBS$Sueldo, datBS$Puesto, mean)),0)
maximo <- round(c(tapply(datBS$Sueldo, datBS$Puesto, max)),0)
minimo <- round(c(tapply(datBS$Sueldo, datBS$Puesto, min)),0)
mediana<- round(c(tapply(datBS$Sueldo, datBS$Puesto, median)),0)
cota_minima <- mediana*0.65
cota_maxima <- mediana*1.35

datBS_22 <- NULL

datBS_22 <- data.frame(posicion,Presupuesto,Promedio,mediana,minimo,maximo,cota_minima,cota_maxima)
colnames(datBS_22) <- c("Puesto","Presupuesto","Promedio(En soles)","Mediana(En soles)","Minimo(En soles)","Máximo(En soles)","Minimo esperado(En soles)","Máximo esperado(En soles)")
row.names(datBS_22) <- NULL

datBS_22 <- datBS_22 %>% arrange(Presupuesto)
datBS_22$Puesto <- factor(datBS_22$Puesto, levels = datBS_22$Puesto)

p=ggplot(datBS_22, aes(x=Puesto, y=Presupuesto,color = Puesto)) +
...
### ANALISIS DETALLADO DE LAS REMUNERACIONES POR PUESTO DE TRABAJO

```{r }
Tabla = datBS_22
Tabla = Tabla[order(-Tabla$Presupuesto),]
rownames(Tabla) <- Tabla$Puesto


names(Tabla) = c("Puesto","Presupuesto","Promedio (En soles)","Mediana (En soles)","Minimo (En soles)","Máximo (En soles)","Minimo esperado (En soles)","Máximo esperado (En soles)")
Tabla$Puesto <- NULL
Tabla$Presupuesto = paste(round(Tabla$Presupuesto,2), "%", sep="")
Tabla[,2:4]= round(Tabla[,2:4])

needs(DT)
datatable(Tabla , options = list(bPaginate = FALSE,dom = 't',columnDefs = list(list(className = 'dt-center', targets =1:4))))
